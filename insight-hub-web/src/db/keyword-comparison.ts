...
// (voir design.md, Decision "Résolution du second run").
export async function getComparableKeywords(excludeKeyword: string | null): Promise<string[]> {
  const rows = await db
...
export async function getLatestRunIdForKeyword(keyword: string): Promise<number | null> {
  const normalized = keyword.trim().toLowerCase();
  ...
  const rows = await db
    ...
    .from(importRuns)
    .innerJoin(messages, eq(messages.runId, importRuns.id))
    .where(eq(sql`lower(${importRuns.keyword})`, normalized));
