# representative-messages-by-theme-sentiment Specification

## Purpose
Donner, pour chaque combinaison thème × catégorie de sentiment, un exemple concret et actionnable : le message le plus engageant déjà classé sous le scope du dernier run d'import et des filtres croisés actifs, sans nouveau calcul IA. Les KPIs du dashboard donnent des chiffres agrégés par thème (`top-themes-restitution`) et par sentiment (`engagement-rate-by-sentiment`), mais aucun ne montre à quoi ressemble concrètement un message pour une combinaison thème × sentiment donnée — un insight actionnable exige toujours un exemple concret cité.
