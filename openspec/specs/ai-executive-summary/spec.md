...
Générer, mettre en cache et restituer sur le dashboard un résumé exécutif en langage naturel produit par IA, construit exclusivement à partir des KPIs déjà calculés (score de sentiment net et sa tendance, thème le plus à risque et sa tendance, répartition plateforme/pays, exemple de message représentatif) — jamais un nouvel appel IA d'analyse sur les messages bruts. Répond aux 3 questions business du PRD (dégradation d'image, thème le plus négatif, exposition plateforme/pays) en appliquant la règle d'insight actionnable (chiffre + comparaison + exemple concret), avec un appel Gemini au plus par périmètre de consultation et une dégradation gracieuse en cas d'échec.
