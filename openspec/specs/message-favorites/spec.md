...
## Purpose
Permettre de marquer et démarquer des messages comme favoris depuis le dashboard, sans notion d'utilisateur connecté (état global), avec une réflexion optimiste dans l'interface et une persistance en base de données. Proposer un filtre "favoris uniquement" combinable avec la recherche plein texte et les filtres croisés existants, restreint à l'affichage des favoris du dernier run d'import sans jamais supprimer le marquage favori des runs antérieurs.
