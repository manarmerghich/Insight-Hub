# engagement-weighted-sentiment-score Specification

## Purpose
Calculer un score de sentiment net alternatif, où chaque message pèse selon son engagement (`1 + likes + retweets`) plutôt que de compter à égalité avec les autres, en complément du score net à comptage égal déjà existant (`net-sentiment-score`) — pour distinguer un sentiment mesuré fréquent mais peu relayé d'un sentiment rare mais très partagé.
