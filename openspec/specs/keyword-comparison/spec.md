...
Le système SHALL proposer, sur le dashboard, un sélecteur listant les mots-clés déjà importés ayant au moins un run avec au moins un message, à l'exclusion du mot-clé du dernier run d'import (comparaison insensible à la casse).

#### Scenario: Plusieurs mots-clés importés
- **WHEN** l'utilisateur consulte le dashboard et que plusieurs mots-clés distincts ont déjà été importés
- **THEN** le sélecteur liste chacun de ces mots-clés, à l'exception de celui du dernier run d'import

#### Scenario: Un seul mot-clé jamais importé
- **WHEN** l'utilisateur consulte le dashboard et qu'aucun autre mot-clé que celui du dernier run d'import n'a été importé
- **THEN** le sélecteur reste visible mais désactivé, avec un message explicite invitant à importer un second mot-clé pour activer la comparaison
